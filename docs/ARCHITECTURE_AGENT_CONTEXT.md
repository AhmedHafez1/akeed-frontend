...
### Onboarding (Shared Route, Mode-Specific Flow)

- URL is `/{locale}/onboarding`, implemented in `src/app/[locale]/(embedded)/onboarding/page.tsx`.
- The page branches by runtime mode:
  - Embedded: renders Polaris-based Shopify onboarding flow.
  - Standalone: renders `OnboardingContainer` (Tailwind wizard via `useOnboarding`).

Embedded flow details:

- Step 1: welcome.
- Step 2: configuration (`storeName`, `defaultLanguage`, `isAutoVerifyEnabled`).
- Step 3: billing activation.
- Backend calls:
  - `GET /api/onboarding/state` (prefill + status check)
  - `PATCH /api/onboarding/settings`
  - `POST /api/onboarding/billing` (returns `confirmationUrl` for Shopify redirect)
- Redirect behavior:
  - If onboarding is already completed, route redirects to `/{locale}/dashboard` while preserving query params.
  - Root page and dashboard page both re-check onboarding state in embedded mode and redirect pending merchants back to onboarding.

Standalone flow details:

- Step 1: create org (`POST /api/organizations`).
- Step 2: platform selection (UI selection only; controls step branching).
- Step 3 (optional): WhatsApp credential capture (`PATCH /api/organizations/current`).
- Step 4: completion + dashboard navigation.
...
- Standalone platform selection in onboarding is currently a UI branching choice; only WhatsApp credential submission persists data.
